...
3. **Step labels for verifier training (automatic where possible)**
   ...
   * **WebArena:** define progress signal using evaluation script components (exact match / must-include / fuzzy match etc.) and intermediate subgoals; WebArena’s evaluation is designed for functional correctness. 
...
* On perturbed tasks: **worst-seed success** improves by **≥ +10–15 absolute points** over SLM-only BC, while using **≤ 30%** of the LLM calls of LLM-only.
...
2. **LLM-only ReAct** (reason+act prompting). ReAct improves interactive success in WebShop and compares to act-only. 
3. **SLM-only**:
...
2. LLM-only + multi-sample rerank (best-of-N)
3. SLM-only fine-tune on SWE-bench train (repo-disjoint split noted). 
4. SLM + verifier rerank (SCORE-style oversample-then-rerank). 
...
* **Contamination checks**:
  ...
  * For SWE-bench: deduplicate against training corpora when possible; at minimum, track whether the model reproduces known PR patches verbatim (hash-based).
