...
for tag in "${MODEL_TAGS[@]}"; do
    ...
    output_dir="outputs/verifier/${BENCHMARK}_noisy_${tag}"
    logfile="${LOGDIR}/verifier_${tag}.log"
    ...
    if ${DRY_RUN}; then
        ...
        if ! "${MERGE_CMD[@]}" 2>&1 | tee -a "${logfile}"; then
...
if [[ ${#SUCCEEDED[@]} -gt 0 ]]; then
    ...
    for tag in "${SUCCEEDED[@]}"; do
        echo "    ✓ ${tag}"
        echo "        verifier:  outputs/verifier/${BENCHMARK}_noisy_${tag}/"
...
if [[ ${#FAILED[@]} -gt 0 ]]; then
    ...
    for tag in "${FAILED[@]}"; do
        echo "    ✗ ${tag}  →  logs/verifier_${tag}.log"
    done
fi
